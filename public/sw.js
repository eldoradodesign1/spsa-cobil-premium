const CACHE = "spsa-cobil-shell-v7";
self.addEventListener("install", (event) => event.waitUntil(caches.open(CACHE).then((cache) => cache.addAll(["./", "./manifest.webmanifest", "./icons/spsa-cobil-logo.png"])).then(() => self.skipWaiting())));
